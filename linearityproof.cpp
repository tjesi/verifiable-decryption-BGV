...
void ProveLinearRelation(LinProof &proof, const ComKey &comKey, const Commitment &X1, const Commitment &X2, const ZZ_pE &alpha, const ZZ_pE &beta){
    Vec<ZZ_pE> y1, y2; y1.SetLength(3), y2.SetLength(3);
    bool success = false;
    while (!success){
      ...
      proof.u = alpha*(y1[1] + y1[2]*comKey.B2[2])
        - (y2[1] + y2[2]*comKey.B2[2]);
      SampleChallenge(proof.c);
      success = RejectionSampling();}
    for(int j = 0; j < 3; j++){
      proof.z1[j] = y1[j] + proof.c * X1.r[j];
      proof.z2[j] = y2[j] + proof.c * X2.r[j];}
}

void ProveLinearRelations(Vec<LinProof> &linProof, const Vec<Commitment> &noiseCom, const ComKey comKey, const Commitment keyCom, const Vec<Ciphertext> ciphertexts, const Vec<ZZ_pE> messages){
  for(int j = 0; j < tau; j++){
...
void VerifyLinearRelation(bool &b, const LinProof &proof, const ComKey &comKey,
  ...
  if ((alpha*(proof.z1[1] + proof.z1[2]*comKey.B2[2]) - (proof.z2[1] +
    proof.z2[2]*comKey.B2[2])) != ((alpha*X1[1]+beta-X2[1]))*proof.c + proof.u) {b = false;}
}
